module @module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  stream.executable private @main$async_dispatch_0 {
    stream.executable.export public @main$async_dispatch_0_matmul_128x128x128_f32 workgroups() -> (index, index, index) {
      ...
      stream.return %x, %y, %z : index, index, index loc("ir_00_torch_input.mlir":6:10)
    } loc("ir_00_torch_input.mlir":6:10)
    builtin.module {
      func.func @main$async_dispatch_0_matmul_128x128x128_f32(%arg0: !stream.binding {stream.alignment = 64 : index} loc("ir_00_torch_input.mlir":3:19), %arg1: !stream.binding {stream.alignment = 64 : index} loc(fused["ir_00_torch_input.mlir":4:46, "ir_00_torch_input.mlir":9:10]), %arg2: !stream.binding {stream.alignment = 64 : index} loc("ir_00_torch_input.mlir":6:10)) {
        ...
        %0 = stream.binding.subspan %arg0[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<128x128xf32>> loc("ir_00_torch_input.mlir":3:19)
        %1 = stream.binding.subspan %arg1[%c512] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<128x128xf32>> loc("ir_00_torch_input.mlir":4:46)
        %2 = stream.binding.subspan %arg1[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<128xf32>> loc("ir_00_torch_input.mlir":9:10)
        %3 = stream.binding.subspan %arg2[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<128x128xf32>> loc("ir_00_torch_input.mlir":6:10)
