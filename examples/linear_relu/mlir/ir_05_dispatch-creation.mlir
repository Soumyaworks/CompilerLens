module @module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  util.func public @main$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    %cst = arith.constant dense_resource<torch_tensor_128_torch.float32> : tensor<128xf32> loc("ir_00_torch_input.mlir":9:10)
    %cst_0 = arith.constant dense_resource<__auto.constant_128_128_torch.float32> : tensor<128x128xf32> loc("ir_00_torch_input.mlir":4:46)
    %0 = hal.tensor.import wait(%arg1) => %arg0 : !hal.buffer_view -> tensor<128x128xf32> loc("ir_00_torch_input.mlir":3:19)
    %1 = flow.dispatch.workgroups(%0, %cst_0, %cst) : (tensor<128x128xf32>, tensor<128x128xf32>, tensor<128xf32>) -> tensor<128x128xf32> =
        (%arg3: !iree_tensor_ext.dispatch.tensor<readonly:tensor<128x128xf32>>, %arg4: !iree_tensor_ext.dispatch.tensor<readonly:tensor<128x128xf32>>, %arg5: !iree_tensor_ext.dispatch.tensor<readonly:tensor<128xf32>>, %arg6: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<128x128xf32>>) {
      %cst_1 = arith.constant 0.000000e+00 : f32 loc("ir_00_torch_input.mlir":6:10)
      %4 = iree_tensor_ext.dispatch.tensor.load %arg3, offsets = [0, 0], sizes = [128, 128], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<128x128xf32>> -> tensor<128x128xf32> loc("ir_00_torch_input.mlir":6:10)
      %5 = iree_tensor_ext.dispatch.tensor.load %arg4, offsets = [0, 0], sizes = [128, 128], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<128x128xf32>> -> tensor<128x128xf32> loc("ir_00_torch_input.mlir":6:10)
      %6 = iree_tensor_ext.dispatch.tensor.load %arg5, offsets = [0], sizes = [128], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<128xf32>> -> tensor<128xf32> loc("ir_00_torch_input.mlir":6:10)
      %7 = tensor.empty() : tensor<128x128xf32> loc("ir_00_torch_input.mlir":6:10)
      %8 = linalg.fill ins(%cst_1 : f32) outs(%7 : tensor<128x128xf32>) -> tensor<128x128xf32> loc("ir_00_torch_input.mlir":6:10)
      ...
      ^bb0(%in: f32 loc("ir_00_torch_input.mlir":6:10), %in_2: f32 loc("ir_00_torch_input.mlir":9:10), %out: f32 loc("ir_00_torch_input.mlir":10:10)):
        %11 = arith.addf %in, %in_2 : f32 loc("ir_00_torch_input.mlir":9:10)
        %12 = arith.cmpf ugt, %11, %cst_1 : f32 loc("ir_00_torch_input.mlir":10:10)
        %13 = arith.select %12, %11, %cst_1 : f32 loc("ir_00_torch_input.mlir":10:10)
        linalg.yield %13 : f32 loc("ir_00_torch_input.mlir":10:10)
      } -> tensor<128x128xf32> loc("ir_00_torch_input.mlir":10:10)
      iree_tensor_ext.dispatch.tensor.store %10, %arg6, offsets = [0, 0], sizes = [128, 128], strides = [1, 1] : tensor<128x128xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<128x128xf32>> loc("ir_00_torch_input.mlir":6:10)
      flow.return loc("ir_00_torch_input.mlir":6:10)
